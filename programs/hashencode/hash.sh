...
read text
if [[ $args == *'md5'* ]]; then
hash=$(printf "${text}" | md5sum | awk '{print $1}')
elif [[ $args == *'sha1'* ]]; then
hash=$(printf "${text}" | sha1sum | awk '{print $1}')
elif [[ $args == *'sha224'* ]]; then
hash=$(printf "${text}" | sha224sum | awk '{print $1}')
elif [[ $args == *'sha256'* ]]; then
hash=$(printf "${text}" | sha256sum | awk '{print $1}')
elif [[ $args == *'sha384'* ]]; then
hash=$(printf "${text}" | sha384sum | awk '{print $1}')
elif [[ $args == *'sha512'* ]]; then
hash=$(printf "${text}" | sha512sum | awk '{print $1}')
else
